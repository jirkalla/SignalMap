...
- **Every delete action gets a client-side `confirm()` guard** on the form: `data-confirm="{{ t('<entity>.delete_confirm') }}"`, handled by the shared delegated `submit` listener in `base.html` — never a bare delete button, and never `onsubmit="return confirm('{{ t(...) }}')"` (that pattern silently breaks the moment the translated text contains an apostrophe, e.g. English "can't" — the browser HTML-decodes the attribute before JS sees it, so the string literal closes early and the whole handler fails to parse).
