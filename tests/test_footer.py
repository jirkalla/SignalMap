...
itself is fine for everyone. The globals are read once at import, so each test patches them.
"""
...
def baked_build(monkeypatch):
    ...
    monkeypatch.setitem(templates.env.globals, "git_sha", FAKE_SHA)
    monkeypatch.setitem(templates.env.globals, "build_time", "2026-09-23T06:00:00Z")
    monkeypatch.setitem(templates.env.globals, "app_environment", "development")
...
def test_logged_in_footer_without_build_args_omits_sha(authed_client: TestClient, monkeypatch):
    ...
    monkeypatch.setitem(templates.env.globals, "git_sha", "")
    response = authed_client.get("/clients")
    assert response.status_code == 200
...
def test_production_hides_environment_badge(authed_client: TestClient, baked_build, monkeypatch):
    monkeypatch.setitem(templates.env.globals, "app_environment", "production")
    response = authed_client.get("/clients")
