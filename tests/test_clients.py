...
def test_create_list_edit_detail_flow(client: TestClient):
...
def test_delete_blocked_when_a_run_exists_under_the_client(client: TestClient, db_session: Session, seed: dict):
    client_id = _create_client(client)
...
def test_delete_succeeds_for_a_client_with_no_runs(client: TestClient):
    client_id = _create_client(client, name="Empty Co")
