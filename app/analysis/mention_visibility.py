...
def _match_positions(rendered_text: str, candidates: list[str]) -> tuple[set[int], list[str]]:
    """Union of match-start positions across all candidates, plus which candidates matched.

    Positions are deduped across candidates (design decision 5) so an alias
    that overlaps the client's own name doesn't double-count the same
    occurrence.
    """
    ...
    for candidate in candidates:
        ...
        candidate_positions = [m.start() for m in re.finditer(pattern, rendered_text, re.IGNORECASE)]
        if candidate_positions:
            matched_terms.append(candidate)
            positions.update(candidate_positions)
    return positions, matched_terms
...
class MentionVisibilityRunner:
    ...
    def run(self, rendered_text: str | None, citations: list[Citation], client: Client) -> dict[str, Any]:
        ...
        if rendered_text:
            positions, matched_terms = _match_positions(rendered_text, candidates)
        else:
            positions, matched_terms = set(), []

        cited_domains = _matching_citation_domains(client.domain, citations)

        return {
            "text_mentioned": len(positions) > 0,
            "mention_count": len(positions),
            "first_mention_position": min(positions) if positions else None,
